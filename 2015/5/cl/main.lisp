...
(defun contains-two-letters-twice-p (line)
  (cl-ppcre:scan "((?:\\w){2}).*?\\1" line))
(defun contains-one-letters-one-apart-p (line)
  (cl-ppcre:scan "(\\w).\\1" line))
(defun even-nicer-line-p (line)
  (and (contains-two-letters-twice-p line)
       (contains-one-letters-one-apart-p line)))
  ...
  (let ((nice-words 0))
    (with-open-file (in "../input.txt")
      (loop for line = (read-line in nil nil)
	    while line
	    do (when (even-nicer-line-p line)
		 (incf nice-words))))
    nice-words))
